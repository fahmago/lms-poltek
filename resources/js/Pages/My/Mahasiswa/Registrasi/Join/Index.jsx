...
const Create = () => {
    const { errors, kelas, kelasTerpilih, flash } = usePage().props; 

    // console.log(kelas);

    const [kelasHarianId, setKodeKelas] = useState('');  
    ...
    const storeRegistrasiKelas = async (e) => {
        ...
        Inertia.post(route('mhs.join.store'), {
            id: kelasHarianId,
            ...
            onSuccess: () => {
                ...
                ToastNotification({
                    icon: 'success',
                    title: flash.success || 'Anda Berhasil Join ke Kelas!',
                    timer: 2000,
                });
                resetForm();
            },
                                    ...
                                    {/* Select: Kode Kelas */}                        
                                     <SF3
                                        ...
                                        options={kelas.map((kls) => ({
                                            value: kls.id,
                                            label: kelasTerpilih.includes(kls.id)
                                                ? `${kls.tahun} - ${kls.nama_kelas} - ${kls.dosen.user.name} - Semester ${kls.semester}`
                                                : `${kls.tahun} - ${kls.nama_kelas} - ${kls.dosen.user.name} - Semester ${kls.semester}`,
                                            isDisabled: kelasTerpilih.includes(kls.id), // Disable jika sudah terdaftar
                                        }))}
                                        placeholder="Cari atau pilih kelas"
                                        error={errors.id} // Error validasi dari server
                                    />
