...
const SertifikatCard = ({ sertifikat }) => { // Ganti nama & prop
    const deadline = formatDeadline(sertifikat.batas_waktu);
    return (
        // GANTI: route 'mhs.buku.show' -> 'mhs.sertifikat.show' & param
        <Link href={route('mhs.sertifikat.show', { sertifikat: sertifikat.uuid })} className="block bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
            <div className="p-6">
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-blue-600">{sertifikat.nama_kelas}</p>
                    </div>
                    <StatusBadge status={sertifikat.status} />
                </div>
                <h3 className="mt-2 text-lg font-bold text-gray-900 leading-tight">{sertifikat.judul}</h3>
            </div>
...
// Halaman Utama Daftar Sertifikat
export default function Index() {
    const { filters, statusCounts } = usePage().props;
    
    // GANTI: 'bukuData' -> 'sertifikatData'
    ...
    // GANTI: 'fetchBukus' -> 'fetchSertifikats' (Nama fungsi)
    const fetchSertifikats = (page = 1, status = '') => {
        setIsLoading(true);
        // GANTI: route 'mhs.buku.index' -> 'mhs.sertifikat.index'
        axios.get(route('mhs.sertifikat.index'), {
            params: { page, status: status === 'Semua' ? '' : status },
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(response => {
            setSertifikatData(response.data); // GANTI VARIABEL
        })
        .catch(error => console.error("Gagal memuat data sertifikat:", error)) // GANTI TEKS
        .finally(() => setIsLoading(false));
    };

    // Efek untuk memuat data pertama kali
    useEffect(() => {
        fetchSertifikats(1, activeFilter); // GANTI FUNGSI
    }, []);

    // Handler untuk mengubah filter
    const handleFilterChange = (status) => {
        setActiveFilter(status);
        fetchSertifikats(1, status); // GANTI FUNGSI
    };

    // Handler untuk mengubah halaman
    const handlePageChange = (newPage) => {
        fetchSertifikats(newPage, activeFilter); // GANTI FUNGSI
    };
    ...
    const renderContent = () => {
        ...
        // GANTI: 'sertifikatData'
        if (sertifikatData && sertifikatData.data.length > 0) {
            return (
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                        {/* GANTI: 'sertifikatData' -> 'SertifikatCard' */}
                        {sertifikatData.data.map(sertifikat => <SertifikatCard key={sertifikat.id} sertifikat={sertifikat} />)}
                    </div>
                    <AxiosPagination data={sertifikatData} onPageChange={handlePageChange} isLoading={isLoading} />
                </>
            );
        }
        ...
        return (
            <div className="text-center py-20 bg-white rounded-lg shadow-md mt-6">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">Yey, Semua Sertifikat Selesai!</h3>
                <p className="mt-1 text-sm text-gray-500">Tidak ada tugas sertifikat yang sesuai dengan filter ini.</p>
            </div>
        );
    };
    ...
    return (
        <MyLayout>
            {/* GANTI: Head title */}
            <Head title="Daftar Tugas Sertifikat" />
            <div className="space-y-6">
                <div className="mb-6">
                    {/* GANTI: Teks Header */}
                    <div className="text-2xl font-semibold text-gray-800">Tugas Sertifikat Saya</div>
                    <p className="mt-1 text-gray-600">Daftar semua tugas sertifikat yang perlu Anda kerjakan dan yang sudah selesai.</p>
                </div>
