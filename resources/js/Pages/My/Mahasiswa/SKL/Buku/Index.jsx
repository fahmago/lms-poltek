...
const BukuCard = ({ buku }) => { // Ganti nama & prop
    const deadline = formatDeadline(buku.batas_waktu);
    return (
        // GANTI: route 'mhs.portofolio.show' -> 'mhs.buku.show' & param
        <Link href={route('mhs.buku.show', { buku: buku.uuid })} className="block bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
            <div className="p-6">
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-blue-600">{buku.nama_kelas}</p>
                    </div>
                    <StatusBadge status={buku.status} />
                </div>
                <h3 className="mt-2 text-lg font-bold text-gray-900 leading-tight">{buku.judul}</h3>
            </div>
...
export default function Index() {
    ...
    // GANTI: 'portofolioData' -> 'bukuData'
    const [bukuData, setBukuData] = useState(null);
    const [isLoading, setIsLoading] = useState(true);
    const [activeFilter, setActiveFilter] = useState(filters.status || 'Semua');

    // GANTI: 'fetchPortofolios' -> 'fetchBukus' (Nama fungsi)
    const fetchBukus = (page = 1, status = '') => {
        setIsLoading(true);
        // GANTI: route 'mhs.portofolio.index' -> 'mhs.buku.index'
        axios.get(route('mhs.buku.index'), {
            params: { page, status: status === 'Semua' ? '' : status },
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(response => {
            setBukuData(response.data); // GANTI VARIABEL
        })
        .catch(error => console.error("Gagal memuat data buku:", error)) // GANTI TEKS
        .finally(() => setIsLoading(false));
    };

    // Efek untuk memuat data pertama kali
    useEffect(() => {
        fetchBukus(1, activeFilter); // GANTI FUNGSI
    }, []);

    // Handler untuk mengubah filter
    const handleFilterChange = (status) => {
        setActiveFilter(status);
        fetchBukus(1, status); // GANTI FUNGSI
    };

    // Handler untuk mengubah halaman
    const handlePageChange = (newPage) => {
        fetchBukus(newPage, activeFilter); // GANTI FUNGSI
    };
    ...
    const renderContent = () => {
        ...
        // GANTI: 'bukuData'
        if (bukuData && bukuData.data.length > 0) {
            return (
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                        {/* GANTI: 'bukuData' -> 'BukuCard' */}
                        {bukuData.data.map(buku => <BukuCard key={buku.id} buku={buku} />)}
                    </div>
                    <AxiosPagination data={bukuData} onPageChange={handlePageChange} isLoading={isLoading} />
                </>
            );
        }
        ...
        return (
            <div className="text-center py-20 bg-white rounded-lg shadow-md mt-6">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">Yey, Semua Buku Selesai!</h3>
                <p className="mt-1 text-sm text-gray-500">Tidak ada tugas buku yang sesuai dengan filter ini.</p>
            </div>
        );
    };
    ...
    return (
        <MyLayout>
            {/* GANTI: Head title */}
            <Head title="Daftar Tugas Buku" />
            <div className="space-y-6">
                <div className="mb-6">
                    {/* GANTI: Teks Header */}
                    <div className="text-2xl font-semibold text-gray-800">Tugas Buku Saya</div>
                    <p className="mt-1 text-gray-600">Daftar semua tugas buku yang perlu Anda kerjakan dan yang sudah selesai.</p>
                </div>
