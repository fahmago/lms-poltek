...
const PortofolioCard = ({ portofolio }) => { // Ganti nama & prop
    const deadline = formatDeadline(portofolio.batas_waktu);
    return (
        // GANTI: route 'mhs.tsem.show' -> 'mhs.portofolio.show' & param
        <Link href={route('mhs.portofolio.show', { portofolio: portofolio.uuid })} className="block bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
            <div className="p-6">
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-blue-600">{portofolio.nama_kelas}</p>
                    </div>
                    <StatusBadge status={portofolio.status} />
                </div>
                <h3 className="mt-2 text-lg font-bold text-gray-900 leading-tight">{portofolio.judul}</h3>
            </div>
...
// Halaman Utama Daftar Portofolio
export default function Index() {
    const { filters, statusCounts } = usePage().props;
    
    // GANTI: 'projectData' -> 'portofolioData'
    ...
    // GANTI: 'fetchProjects' -> 'fetchPortofolios' (Nama fungsi)
    const fetchPortofolios = (page = 1, status = '') => {
        setIsLoading(true);
        // GANTI: route 'mhs.tsem.index' -> 'mhs.portofolio.index'
        axios.get(route('mhs.portofolio.index'), {
            params: { page, status: status === 'Semua' ? '' : status },
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(response => {
            setPortofolioData(response.data); // GANTI VARIABEL
        })
        .catch(error => console.error("Gagal memuat data portofolio:", error)) // GANTI TEKS
        .finally(() => setIsLoading(false));
    };

    // Efek untuk memuat data pertama kali
    useEffect(() => {
        fetchPortofolios(1, activeFilter); // GANTI FUNGSI
    }, []);

    // Handler untuk mengubah filter
    const handleFilterChange = (status) => {
        setActiveFilter(status);
        fetchPortofolios(1, status); // GANTI FUNGSI
    };

    // Handler untuk mengubah halaman
    const handlePageChange = (newPage) => {
        fetchPortofolios(newPage, activeFilter); // GANTI FUNGSI
    };
    ...
    const renderContent = () => {
        ...
        // GANTI: 'portofolioData'
        if (portofolioData && portofolioData.data.length > 0) {
            return (
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                        {/* GANTI: 'portofolioData' -> 'PortofolioCard' */}
                        {portofolioData.data.map(portofolio => <PortofolioCard key={portofolio.id} portofolio={portofolio} />)}
                    </div>
                    <AxiosPagination data={portofolioData} onPageChange={handlePageChange} isLoading={isLoading} />
                </>
            );
        }
        ...
        return (
            <div className="text-center py-20 bg-white rounded-lg shadow-md mt-6">
                ...
                <h3 className="mt-2 text-sm font-medium text-gray-900">Yey, Semua Portofolio Selesai!</h3>
                <p className="mt-1 text-sm text-gray-500">Tidak ada portofolio yang sesuai dengan filter ini.</p>
            </div>
        );
    };
    ...
    return (
        <MyLayout>
            {/* GANTI: Head title */}
            <Head title="Daftar Portofolio" />
            <div className="space-y-6">
                <div className="mb-6">
                    {/* GANTI: Teks Header */}
                    <div className="text-2xl font-semibold text-gray-800">Portofolio Saya</div>
                    <p className="mt-1 text-gray-600">Daftar semua portofolio yang perlu Anda kerjakan dan yang sudah selesai.</p>
                </div>
