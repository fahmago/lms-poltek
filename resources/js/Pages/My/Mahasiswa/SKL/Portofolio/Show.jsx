...
import MyLayout from '@/Layouts/MyLayout'; // Ganti ke path relatif
import { Head, usePage, Link } from '@inertiajs/inertia-react';
import formatDate from '@/Utilities/formatDateTime'; // Ganti ke path relatif
import ReactQuill from 'react-quill';
import 'react-quill/dist/quill.snow.css';
import ToastNotification from '@/Shared/ToastNotification'; // Ganti ke path relatif
// GANTI: Path ke folder KomponenPortofolio
...
export default function Show() {
    ...
    const { portofolio, submission, status, flash } = usePage().props;
    ...
    const startTime = new Date(portofolio.waktu_mulai);
    const deadline = new Date(portofolio.batas_waktu);
    const now = new Date();
    ...
    return (
        <MyLayout>
            {/* GANTI: 'portofolio' -> 'portofolio' */}
            <Head title={portofolio.judul} />

            {/* === WRAPPER === */}
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">

                    {/* === CARD DETAIL PORTOFOLIO === */}
                    <div className="bg-white p-6 rounded-xl shadow-lg h-fit">
                        <div className="flex justify-between items-center mb-4 border-b pb-4">
                            {/* GANTI: 'portofolio' -> 'portofolio' */}
                            <h2 className="text-xl font-bold text-gray-800">{portofolio.judul}</h2>
                        </div>
                        ...
                        <div
                            ...
                            dangerouslySetInnerHTML={{
                                // GANTI: 'portofolio' -> 'portofolio'
                                __html: portofolio.deskripsi,
                            }}
                        ></div>

                        {/* HAPUS: Blok Catatan dihapus karena tidak ada di migrasi portofolio */}
                                ...
                                return (
                                    <div className="text-center py-8">
                                        ...
                                        <h3 className="text-lg font-semibold text-blue-700">
                                            Portofolio Belum Dimulai
                                        </h3>
                                        <p className="text-sm text-gray-500 mt-1 mb-6">
                                            Anda dapat mulai mengirimkan portofolio anda dalam:
                                        </p>
                                        {/* GANTI: 'portofolio' -> 'portofolio' */}
                                        <Countdown targetDate={portofolio.waktu_mulai} />
                                    </div>
                                );
                            }
                                ...
                                return (
                                    <div className="text-center py-8">
                                        ...
                                        <p className="text-sm text-gray-500 mt-1">
                                            Anda tidak mengumpulkan jawaban untuk portofolio ini.
                                        </p>
                                    </div>
                                );
                            }
