...
const formatDeadline = (deadline) => {
    ...
    const formattedDate = deadlineDate.toLocaleString('id-ID', { 
        ...
        hour: '2-digit',
        minute: '2-digit'
    }).replace('.', ':'); // Mengganti format jam 21.59 menjadi 21:59

    return { 
        text: `${formattedDate} WIB`,
        color: "text-gray-500" 
    };
    // return { text: deadlineDate.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }), color: "text-gray-500" };
};
...
const TypeBadge = ({ type }) => {
    ...
    const bgColor = isYoutube ? 'bg-red-100' : 'bg-indigo-100';
    const textColor = isYoutube ? 'text-red-800' : 'text-indigo-800';
    const icon = isYoutube ? 'fab fa-youtube' : 'fa fa-link';
    const text = isYoutube ? 'Video' : 'Link';

    return (
        <span className={`inline-flex items-center px-2.5 py-0.5 text-xs font-semibold rounded-full ${bgColor} ${textColor}`}>
...
// --- KOMPONEN KARTU TUGAS DIPERBARUI ---
            ...
            <div className="p-6">
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-blue-600">{tugas.nama_kelas}</p>
                        <TypeBadge type={tugas.tipe_tugas} />
                    </div>
...
export default function Index() {
    ...
    // Fungsi pusat untuk mengambil data
    ...
    // Efek untuk memuat data pertama kali saat halaman diakses
    useEffect(() => {
        fetchTugas(1, activeFilter);
    }, []);
    ...
    // Konten yang akan ditampilkan: skeleton, data, atau pesan kosong
    const renderContent = () => {
        // --- PERBAIKAN DI SINI ---
        // Jika sedang loading, selalu tampilkan skeleton
        if (isLoading) {
            return (
        ...
        // Jika tidak loading dan ada data
        if (tugasData && tugasData.data.length > 0) {
            return (
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                        {tugasData.data.map(tugas => <TugasCard key={tugas.id} tugas={tugas} />)}
                    </div>
    ...
    return (
        <MyLayout>
            ...
            <div className="space-y-6">
                <div className="mb-6">
                    <div className="text-2xl font-semibold text-gray-800">Tugas Pekanan Saya</div>
