...
const ListJadwal = () => {
    ...
    const rows = useMemo(() => {
        return jadwal.map((item, index) => {
            ...
            const needsAction = sedangWaktunyaPresensi && presensiBelumDiisi;
            
            return {
                // className: needsAction ? 'bg-green-100 animate-pulse font-bold' : '',
                className: needsAction ? 'bg-green-100 font-bold text-green-900 text-xl hover:bg-green-100' : '',
                data: [
                    <span>{index + 1}</span>,
                        ...
                        <div className="flex justify-center">
                            {needsAction ? (
                                ...
                                <span className="text-gray-500">
                                    {presensiBelumDiisi ? "Belum Waktunya" : "Sudah Mengisi"}
                                </span>
                            )}
                        </div>
                    ),
