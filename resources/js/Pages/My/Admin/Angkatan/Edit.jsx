...
const Edit = () => {
    // destruct props "errors" and "angkatan"
    const { errors, angkatan } = usePage().props;
    ...
    // useEffect untuk set initial values ketika component mount
    useEffect(() => {
        if (angkatan) {
            setNamaAngkatan(angkatan.nama_angkatan);
            setKetuaAngkatan(angkatan.ketua_angkatan);
            setTahunAngkatan(angkatan.tahun_angkatan);
        }
    }, [angkatan]);

    // method "updateAngkatan"
    const updateAngkatan = async (e) => {
        e.preventDefault();
        setIsLoading(true);

        Inertia.put(route('my.angkatans.update', angkatan.uuid), {
            ...
            onSuccess: () => {
                ...
                ToastNotification({
                    icon: 'success',
                    title: 'Berhasil update angkatan!',
                    timer: 2000,
                });
            },
            onError: () => {
    ...
    return (
        <>
            <Head>
                <title>eLearning - Edit Angkatan</title>
            </Head>
            <MyLayout>
                <div className="mt-4">
                    <div className="bg-white shadow-sm rounded-md">
                        <div className="bg-blue-600 p-4 rounded-t-md">
                            <span className="font-bold text-white tracking-widest">
                                <i className="fa fa-calendar-days mr-2"></i> Edit Angkatan
                            </span>
                        </div>
                        <div className="p-6">
                            <form onSubmit={updateAngkatan}>
