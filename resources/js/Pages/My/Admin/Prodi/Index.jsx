import { Head, Link, usePage } from '@inertiajs/inertia-react';
import React from 'react';
import MyLayout from '../../../../Layouts/MyLayout';
import hasAnyPermission from '../../../../Utilities/Permissions';
import Search from '../../../../Shared/Search';
import DataTable from '../../../../Shared/DataTable';
import Delete from '../../../../Shared/Delete';

const Index = () => {

    const { prodis } = usePage().props;
    
    const headers = ["No.", "Kode Prodi", "Nama Prodi", "Mahasiswa", "Pengajar", "Matkul", "Actions"];

    const rows = prodis.data.map((prodi, index) => [
        index + 1 + (prodis.current_page - 1) * prodis.per_page,
        prodi.kode_prodi,
        prodi.nama_prodi,
        prodi.mahasiswas_count,
        prodi.dosens_count,     
        prodi.matkuls_count,  
        (
            <div className="flex justify-center">
                {hasAnyPermission(['prodis.edit', 'prodis.delete']) ? (
                    <>
                        {hasAnyPermission(['prodis.edit']) && (
                            <Link
                                href={`/my/prodi/${prodi.uuid}/edit`}
                                className="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                                <i className="fa fa-pencil-alt"></i>
                            </Link>
                        )}
                        {hasAnyPermission(['prodis.delete']) && (
                            <Delete URL={'/my/prodi'} id={prodi.uuid} />
                        )}
                    </>
                ) : (
                    <span className="text-red-500 font-semibold">Minta Akses</span>
                )}
            </div>
        )
    ]);

  return (
    <>
        <Head title='eLearning - Data Prodi' />
        <MyLayout>
            <div className="flex flex-col mt-5">
                <div className={`w-full flex flex-col md:flex-row gap-2 items-center ${hasAnyPermission(['prodis.create']) ? 'justify-between' : 'justify-center'}`}>
                    {hasAnyPermission(['prodis.create']) && (
                        <Link href={route('my.prodis.create')}
                            className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-base px-6 py-3 me-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
                        >
                            <i className="fa fa-plus-circle mr-2"></i>
                            Prodi
                        </Link>
                    )}
                    <div className="w-full md:w-3/4 lg:w-3/6">
                        <Search URL={'/my/prodi'} />
                    </div>
                </div>
            </div>
            <DataTable
                headers={headers}
                rows={rows}
                pagination={prodis}
                iconClass="fa fa-building-columns"
                title="Data Prodi"
            />
        </MyLayout>
    </>
  )
}

export default Index
