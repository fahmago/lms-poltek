...
export default function Edit() {
    ...
    const { projectSemester, angkatans, prodis, initialFilters, errors } = usePage().props;
    ...
    // State untuk data form (diisi dengan data project yang sudah ada)
    // GANTI: 'tugasPekanan' -> 'projectSemester'
    const [judul, setJudul] = useState(projectSemester.judul);
    const [deskripsi, setDeskripsi] = useState(projectSemester.deskripsi);
    // GANTI: 'tipeTugas' -> 'catatan'
    const [catatan, setCatatan] = useState(projectSemester.catatan || '');

    // const [waktuMulai, setWaktuMulai] = useState(projectSemester.waktu_mulai.slice(0, 16));
    // const [batasWaktu, setBatasWaktu] = useState(projectSemester.batas_waktu.slice(0, 16));
    const [waktuMulai, setWaktuMulai] = useState(toLocalDatetime(projectSemester.waktu_mulai));
    const [batasWaktu, setBatasWaktu] = useState(toLocalDatetime(projectSemester.batas_waktu));



    const [selectedClasses, setSelectedClasses] = useState(projectSemester.kelas_harians.map(k => k.id));
    ...
    useEffect(() => {
        if (selectedAngkatan && selectedProdi && selectedSemester) {
            ...
            // GANTI: route 'my.tweek.getClasses' -> 'my.project_semester.getClasses'
            axios.get(route('my.project_semester.getClasses'), {
    ...
    const handleSubmit = (e) => {
        ...
        const dataToSend = {
            judul,
            deskripsi,
            catatan, // GANTI: dari 'tipe_tugas'
            waktu_mulai: waktuMulai, 
            batas_waktu: batasWaktu,
            // waktu_mulai: fromLocalDatetime(waktuMulai),
            // batas_waktu: fromLocalDatetime(batasWaktu),
            kode_prodi: selectedProdi,
            kelas_harian_ids: selectedClasses,
        };
        // Gunakan Inertia.put untuk method UPDATE
        // GANTI: route 'my.tweek.update' -> 'my.project_semester.update' & param
        Inertia.put(route('my.project_semester.update', projectSemester.uuid), dataToSend, {
    ...
    return (
        <MyLayout>
            {/* GANTI: title & 'tugasPekanan' -> 'projectSemester' */}
            <Head title={`Edit Project - ${projectSemester.judul}`} />
            <div className="mx-auto py-8">
                <div className="bg-white shadow-md rounded-lg">
                    <div className="bg-green-600 p-4 rounded-t-lg">
                        {/* GANTI: teks */}
                        <span className="font-bold text-white tracking-widest"><i className="fa fa-edit mr-2"></i>Edit Project Semester</span>
                    </div>
                    <form onSubmit={handleSubmit} className="p-8 space-y-8">
                        ...
                        <div className="space-y-6 pt-6 border-t">
                            {/* GANTI: teks */}
                            <h2 className="text-lg font-semibold text-gray-700">2. Detail Project</h2>
                            {/* GANTI: grid-cols-1 dan hapus 'tipe_tugas' */}
                            <div className="grid grid-cols-1 gap-6">
                                <InputField label="Judul Project" type="text" value={judul} onChange={e => setJudul(e.target.value)} error={errors.judul} required />
                                {/* HAPUS: SelectField2 untuk tipe_tugas */}
                            </div>
                            ...
                            {/* TAMBAH: Input untuk Catatan */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Catatan (Opsional)</label>
                                <ReactQuill theme="snow" value={catatan} onChange={setCatatan} className="h-32" />
                        ...
                        <div className="flex justify-between items-center pt-6 border-t">
                            {/* GANTI: route & 'tugasPekanan' -> 'projectSemester' */}
                            {/* <Link href={route('my.project_semester.show', projectSemester.prodi.uuid)} className="text-sm text-gray-600 hover:underline">
                                &larr; Batal
                            </Link> */}
                            <button
                                type="button"
                                onClick={() => {
                                    if (window.history.length > 1) {
                                        ...
                                        Inertia.visit(route('my.project_semester.show', projectSemester.prodi.uuid));
                                    }
                                }}
