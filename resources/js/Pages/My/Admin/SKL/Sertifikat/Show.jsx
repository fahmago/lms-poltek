...
const Index = () => {
    ...
    // --- PERMISSIONS ---
    const canShow = hasAnyPermission(['sertifikat.show']);
    const canEdit = hasAnyPermission(['sertifikat.edit']);
    const canDelete = hasAnyPermission(['sertifikat.delete']);
    ...
    const handleFilterChange = (key, value) => {
        ...
        Inertia.get(route('my.sertifikat.show', { uuid: prodi.uuid }), newFilter, { preserveState: true, preserveScroll: true, replace: true });
    };

    const handleResetFilter = () => {
        setSelectedTahun('');
        setSelectedSemester('');
        Inertia.get(route('my.sertifikat.show', { uuid: prodi.uuid }), {}, { preserveState: true, preserveScroll: true, replace: true });
    };

    const handleSelectAll = (e) => {
        if (e.target.checked) {
            setSelectedIds(sertifikats.data.map(s => s.uuid));
        } else {
            setSelectedIds([]);
        }
    };
    ...
    const handleSubmitBulk = () => {
        post(route('my.sertifikat.bulk_update_deadline'), {
            onSuccess: () => {
    ...
    // --- STATS CALCULATION ---
    const stats = useMemo(() => {
        const total = sertifikats.data.length;
        if (total === 0) return null;
        const totalAvgProgress = sertifikats.data.reduce((acc, curr) => acc + curr.progress.percentage, 0) / total;
    ...
    const headers = [
        <div className="flex justify-center" key="check-head">
            <input
                ...
                checked={sertifikats.data.length > 0 && selectedIds.length === sertifikats.data.length}
                className="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 cursor-pointer"
            />
        </div>,
        "JUDUL SERTIFIKAT & KELAS", 
        "JADWAL PENGERJAAN", 
        "PROGRESS", 
        "AKSI"
    ];
    ...
    const rows = sertifikats.data.map((sertifikat, index) => {
        const isExpired = new Date(sertifikat.batas_waktu) < new Date();
        
        // Ambil semester dari kelas pertama
        const semester = sertifikat.kelas_harians && sertifikat.kelas_harians.length > 0 
            ? sertifikat.kelas_harians[0].semester 
            : null;

        return [
            // 1. Checkbox
            <div className="flex justify-center" key={`check-${sertifikat.uuid}`}>
                <input
                    type="checkbox"
                    checked={selectedIds.includes(sertifikat.uuid)}
                    onChange={() => handleSelectRow(sertifikat.uuid)}
                    className="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 cursor-pointer"
                />
            </div>,
            ...
            <div className="flex flex-col max-w-sm py-2">
                ...
                <div className="mt-2 flex items-center gap-2">
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800 border border-gray-200">
                        ...
                        {sertifikat.kelas_harians_count} Kelas
                    </span>
                </div>
            </div>,
                ...
                <div className="flex items-center gap-2 group">
                    ...
                    <div className="flex flex-col pb-2">
                        ...
                        <span className="text-xs text-gray-600 font-medium font-mono">
                           {sertifikat.formatted_waktu_mulai}
                        </span>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                     ...
                     <div className="flex flex-col">
                        ...
                        <span className={`text-sm font-bold font-mono ${isExpired ? 'text-red-600' : 'text-gray-800'}`}>
                           {sertifikat.formatted_batas_waktu}
                        </span>
                        {isExpired && (
            ...
            <div className="w-full min-w-[160px] px-2">
                <div className="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <span className="font-bold text-gray-800 bg-gray-100 px-1.5 rounded">
                        {sertifikat.progress.total_submissions} / {sertifikat.progress.total_students}
                    </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3 shadow-inner">
                    <div
                        className={`h-3 rounded-full transition-all duration-700 ease-out shadow-sm ${
                            sertifikat.progress.percentage >= 80 ? 'bg-gradient-to-r from-emerald-400 to-emerald-600' : 
                            sertifikat.progress.percentage >= 50 ? 'bg-gradient-to-r from-amber-300 to-amber-500' : 
                ...
                <div className="text-right mt-1">
                     <span className={`text-xs font-bold ${
                         sertifikat.progress.percentage >= 80 ? 'text-emerald-700' : 
                         sertifikat.progress.percentage >= 50 ? 'text-amber-700' : 'text-rose-700'
                     }`}>
                         {Math.round(sertifikat.progress.percentage)}% Selesai
                     </span>
                </div>
            </div>,
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div>
                        <div className="flex items-center text-sm text-gray-500 mb-2 font-medium">
                             <Link href={route('my.sertifikat.index')} className="hover:text-indigo-600 transition-colors">
                                <i className="fa fa-folder-open-o mr-1"></i> Tugas Sertifikat
                             </Link>
                        ...
                        <p className="text-gray-500 mt-2 text-sm max-w-2xl">
                            Kelola daftar tugas sertifikat, pantau progress mahasiswa, dan sesuaikan tenggat waktu.
                        </p>
                    </div>
                    ...
                    {stats && (
                        <div className="flex gap-4 min-w-max">
                            <div className="flex flex-col items-center justify-center px-6 py-3 bg-white border border-indigo-100 rounded-xl shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <span className="text-xs font-bold text-gray-500 uppercase tracking-wider mt-1">Total Sertifikat</span>
                            </div>
