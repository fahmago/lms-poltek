...
import TugasPekananFilter from '../../Pekanan/TugasPekanan/KomponenPekanan/TugasPekananFilter'; // Komponen filter yang sama

const Index = () => {
    // Properti ini (prodis, angkatans, dll) akan dikirim dari SertifikatController
    const { prodis, angkatans, currentFilters, flash } = usePage().props;

    // State filter awal (Sama)
    const [filters, setFilters] = useState({
        tahun_angkatan: currentFilters.tahun_angkatan || '',
        semester: currentFilters.semester || '',
    });

    // Ref untuk mendeteksi render pertama (Sama)
    const isFirstRender = useRef(true);

    // Notifikasi flash message (Sama)
    useEffect(() => {
    ...
    // Jalankan Inertia GET (Ganti route)
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            if (!filters.tahun_angkatan && !filters.semester) {
                Inertia.get(route('my.sertifikat.index'), {}, { // Ganti route name
                    preserveState: true,
                    preserveScroll: true,
                    replace: true,
                });
            ...
            Inertia.get(route('my.sertifikat.index'), filters, { // Ganti route name
                preserveState: true,
                preserveScroll: true,
                replace: true,
            });
    ...
    // Reset filter ke keadaan awal (Ganti route)
    const handleReset = () => {
    ...
    const headers = ["No.", "Kode Prodi", "Nama Prodi", "Total Sertifikat", "Progress", "Aksi"]; // Ganti "Total Buku"

    const rows = prodis.data.map((prodi, index) => [
        prodis.from + index,
        prodi.kode_prodi,
        prodi.nama_prodi,
        prodi.sertifikats_count, // Ganti properti count
        (
        ...
        (
            <div className="flex justify-center">
                {hasAnyPermission(['sertifikat.show']) ? ( // Ganti permission
                    <>
                        <Link
                            href={route('my.sertifikat.show', { uuid: prodi.uuid })} // Ganti route name
    ...
    return (
        <>
            <Head title='eLearning - Sertifikat Prodi' /> {/* Ganti title */}
            <MyLayout>
                <div className="flex flex-col ">
                    <div className="w-full flex flex-col md:flex-row gap-2 items-center justify-between">
                        {hasAnyPermission(['sertifikat.create']) && ( // Ganti permission
                            <Link
                                href={route('my.sertifikat.create')} // Ganti route name
                            ...
                            >
                                ...
                                Tugas Sertifikat Baru {/* Ganti teks */}
                            </Link>
                        )}
                        <div className="w-full md:w-3/4 lg:w-3/6">
                            <Search
                                URL={route('my.sertifikat.index')} // Ganti route name
