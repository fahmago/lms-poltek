...
const Index = () => {
    // Properti ini (prodis, angkatans, dll) akan dikirim dari BukuController
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            if (!filters.tahun_angkatan && !filters.semester) {
                Inertia.get(route('my.buku.index'), {}, { // Ganti route name
                    preserveState: true,
                    preserveScroll: true,
                    replace: true,
                });
            ...
            Inertia.get(route('my.buku.index'), filters, { // Ganti route name
                preserveState: true,
                preserveScroll: true,
                replace: true,
            });
    ...
    const headers = ["No.", "Kode Prodi", "Nama Prodi", "Total Buku", "Progress", "Aksi"]; // Ganti "Total Portofolio"

    const rows = prodis.data.map((prodi, index) => [
        prodis.from + index,
        prodi.kode_prodi,
        prodi.nama_prodi,
        prodi.bukus_count, // Ganti properti count
        (
        ...
        (
            <div className="flex justify-center">
                {hasAnyPermission(['buku.show']) ? ( // Ganti permission
                    <>
                        <Link
                            href={route('my.buku.show', { uuid: prodi.uuid })} // Ganti route name
    ...
    return (
        <>
            <Head title='eLearning - Buku Prodi' /> {/* Ganti title */}
            <MyLayout>
                <div className="flex flex-col ">
                    <div className="w-full flex flex-col md:flex-row gap-2 items-center justify-between">
                        {hasAnyPermission(['buku.create']) && ( // Ganti permission
                            <Link
                                href={route('my.buku.create')} // Ganti route name
                            ...
                            >
                                ...
                                Tugas Buku Baru {/* Ganti teks */}
                            </Link>
                        )}
                        <div className="w-full md:w-3/4 lg:w-3/6">
                            <Search
                                URL={route('my.buku.index')} // Ganti route name
