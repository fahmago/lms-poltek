...
export default function Edit() {
    ...
    const { buku, angkatans, prodis, initialFilters, errors } = usePage().props;
    ...
    // State untuk data form (Ganti 'buku')
    const [judul, setJudul] = useState(buku.judul);
    const [deskripsi, setDeskripsi] = useState(buku.deskripsi);
    const [waktuMulai, setWaktuMulai] = useState(toLocalDatetime(buku.waktu_mulai));
    const [batasWaktu, setBatasWaktu] = useState(toLocalDatetime(buku.batas_waktu));
    const [selectedClasses, setSelectedClasses] = useState(buku.kelas_harians.map(k => k.id));
    ...
    useEffect(() => {
        if (selectedAngkatan && selectedProdi && selectedSemester) {
            ...
            // GANTI: route 'my.portofolio.getClasses' -> 'my.buku.getClasses'
    ...
    const handleSubmit = (e) => {
        ...
        // GANTI: route 'my.portofolio.update' -> 'my.buku.update' & param
        Inertia.put(route('my.buku.update', {buku: buku.uuid}), dataToSend, {
    ...
    return (
        <MyLayout>
            {/* GANTI: title & 'buku' -> 'buku' */}
            <Head title={`Edit Tugas Buku - ${buku.judul}`} />
            <div className="mx-auto py-8">
                <div className="bg-white shadow-md rounded-lg">
                    <div className="bg-green-600 p-4 rounded-t-lg">
                        {/* GANTI: teks & icon */}
                        <span className="font-bold text-white tracking-widest"><i className="fa fa-book mr-2"></i>Edit Tugas Buku</span>
                    </div>
                            ...
                            <button
                                ...
                                onClick={() => {
                                    if (window.history.length > 1) {
                                        ...
                                        Inertia.visit(route('my.buku.show', {uuid: buku.prodi.uuid}));
                                    }
                                }}
