...
const Index = () => {
    ...
    // --- PERMISSIONS ---
    const canShow = hasAnyPermission(['buku.show']);
    const canEdit = hasAnyPermission(['buku.edit']);
    const canDelete = hasAnyPermission(['buku.delete']);
    ...
    const handleFilterChange = (key, value) => {
        ...
        Inertia.get(route('my.buku.show', { uuid: prodi.uuid }), newFilter, { preserveState: true, preserveScroll: true, replace: true });
    };

    const handleResetFilter = () => {
        setSelectedTahun('');
        setSelectedSemester('');
        Inertia.get(route('my.buku.show', { uuid: prodi.uuid }), {}, { preserveState: true, preserveScroll: true, replace: true });
    };

    const handleSelectAll = (e) => {
        if (e.target.checked) {
            setSelectedIds(bukus.data.map(b => b.uuid));
        } else {
            setSelectedIds([]);
        }
    };
    ...
    const handleSubmitBulk = () => {
        post(route('my.buku.bulk_update_deadline'), {
            onSuccess: () => {
    ...
    // --- STATS CALCULATION ---
    const stats = useMemo(() => {
        const total = bukus.data.length;
        if (total === 0) return null;
        const totalAvgProgress = bukus.data.reduce((acc, curr) => acc + curr.progress.percentage, 0) / total;
    ...
    const headers = [
        <div className="flex justify-center" key="check-head">
            <input
                ...
                checked={bukus.data.length > 0 && selectedIds.length === bukus.data.length}
    ...
    const rows = bukus.data.map((buku, index) => {
        const isExpired = new Date(buku.batas_waktu) < new Date();
        
        // Ambil semester dari kelas pertama
        const semester = buku.kelas_harians && buku.kelas_harians.length > 0 
            ? buku.kelas_harians[0].semester 
            : null;

        return [
            // 1. Checkbox
            <div className="flex justify-center" key={`check-${buku.uuid}`}>
                <input
                    type="checkbox"
                    checked={selectedIds.includes(buku.uuid)}
                    onChange={() => handleSelectRow(buku.uuid)}
                    className="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 cursor-pointer"
                />
            </div>,
            ...
            <div className="flex flex-col max-w-sm py-2">
                ...
                <Link 
                    href={route('my.buku.detail', { buku: buku.uuid })}
                ...
                <div className="mt-2 flex items-center gap-2">
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800 border border-gray-200">
                        ...
                        {buku.kelas_harians_count} Kelas
                    </span>
                </div>
            </div>,
            ...
            <div className="w-full min-w-[160px] px-2">
                <div className="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <span className="font-bold text-gray-800 bg-gray-100 px-1.5 rounded">
                        {buku.progress.total_submissions} / {buku.progress.total_students}
                    </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3 shadow-inner">
                    <div
                        className={`h-3 rounded-full transition-all duration-700 ease-out shadow-sm ${
                            buku.progress.percentage >= 80 ? 'bg-gradient-to-r from-emerald-400 to-emerald-600' : 
                            buku.progress.percentage >= 50 ? 'bg-gradient-to-r from-amber-300 to-amber-500' : 
                ...
                <div className="text-right mt-1">
                     <span className={`text-xs font-bold ${
                         buku.progress.percentage >= 80 ? 'text-emerald-700' : 
                         buku.progress.percentage >= 50 ? 'text-amber-700' : 'text-rose-700'
                     }`}>
                         {Math.round(buku.progress.percentage)}% Selesai
                     </span>
                </div>
            </div>,
            ...
            <div className="flex items-center justify-center gap-2">
                ...
                {canEdit && (
                    <Link 
                        href={route('my.buku.edit', { buku: buku.uuid })} 
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div>
                        <div className="flex items-center text-sm text-gray-500 mb-2 font-medium">
                             <Link href={route('my.buku.index')} className="hover:text-indigo-600 transition-colors">
                                <i className="fa fa-folder-open-o mr-1"></i> Tugas Buku
                             </Link>
                        ...
                        <p className="text-gray-500 mt-2 text-sm max-w-2xl">
                            Kelola daftar tugas buku, pantau progress mahasiswa, dan sesuaikan tenggat waktu.
                        </p>
                    </div>
                    ...
                    {stats && (
                        <div className="flex gap-4 min-w-max">
                            <div className="flex flex-col items-center justify-center px-6 py-3 bg-white border border-indigo-100 rounded-xl shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <span className="text-xs font-bold text-gray-500 uppercase tracking-wider mt-1">Total Buku</span>
                            </div>
