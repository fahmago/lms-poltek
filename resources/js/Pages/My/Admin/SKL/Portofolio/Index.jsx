...
import TugasPekananFilter from '../../Pekanan/TugasPekanan/KomponenPekanan/TugasPekananFilter'; // Anda menggunakan filter yang sama, jadi ini tetap

const Index = () => {
    // Properti ini (prodis, angkatans, dll) akan dikirim dari PortofolioController
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            if (!filters.tahun_angkatan && !filters.semester) {
                Inertia.get(route('my.portofolio.index'), {}, { // Ganti route name
                    preserveState: true,
                    preserveScroll: true,
                    replace: true,
                });
            ...
            Inertia.get(route('my.portofolio.index'), filters, { // Ganti route name
                preserveState: true,
                preserveScroll: true,
                replace: true,
            });
    ...
    const headers = ["No.", "Kode Prodi", "Nama Prodi", "Total Portofolio", "Progress", "Aksi"]; // Ganti "Total Project"

    const rows = prodis.data.map((prodi, index) => [
        prodis.from + index,
        prodi.kode_prodi,
        prodi.nama_prodi,
        prodi.portofolios_count, // Ganti properti count
        (
        ...
        (
            <div className="flex justify-center">
                {hasAnyPermission(['portofolio.show']) ? ( // Ganti permission
                    <>
                        <Link
                            href={route('my.portofolio.show', { uuid: prodi.uuid })} // Ganti route name
    ...
    return (
        <>
            <Head title='eLearning - Portofolio Prodi' /> {/* Ganti title */}
            <MyLayout>
                <div className="flex flex-col ">
                    <div className="w-full flex flex-col md:flex-row gap-2 items-center justify-between">
                        {hasAnyPermission(['portofolio.create']) && ( // Ganti permission
                            <Link
                                href={route('my.portofolio.create')} // Ganti route name
                            ...
                            >
                                ...
                                Portofolio Baru {/* Ganti teks */}
                            </Link>
                        )}
                        <div className="w-full md:w-3/4 lg:w-3/6">
                            <Search
                                URL={route('my.portofolio.index')} // Ganti route name
