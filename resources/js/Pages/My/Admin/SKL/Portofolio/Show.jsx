...
const Index = () => {
    ...
    // --- INERTIA FORM (Untuk Bulk Update) ---
    const { data, setData, post, processing, reset } = useForm({
        ids: [],
        batas_waktu: '',
    });

    // --- PERMISSIONS (Top Level Hooks Fix) ---
    const canShow = hasAnyPermission(['portofolio.show']);
    const canEdit = hasAnyPermission(['portofolio.edit']);
    const canDelete = hasAnyPermission(['portofolio.delete']);
    ...
    const handleFilterChange = (key, value) => {
        ...
        Inertia.get(route('my.portofolio.show', { uuid: prodi.uuid }), newFilter, { preserveState: true, preserveScroll: true, replace: true });
    };

    const handleResetFilter = () => {
        setSelectedTahun('');
        setSelectedSemester('');
        Inertia.get(route('my.portofolio.show', { uuid: prodi.uuid }), {}, { preserveState: true, preserveScroll: true, replace: true });
    };

    const handleSelectAll = (e) => {
        if (e.target.checked) {
            setSelectedIds(portofolios.data.map(p => p.uuid));
        } else {
            setSelectedIds([]);
        }
    };
    ...
    const handleSubmitBulk = () => {
        post(route('my.portofolio.bulk_update_deadline'), {
            onSuccess: () => {
    ...
    // --- STATS CALCULATION (Rata-rata Progress) ---
    const stats = useMemo(() => {
        const total = portofolios.data.length;
        if (total === 0) return null;
        const totalAvgProgress = portofolios.data.reduce((acc, curr) => acc + curr.progress.percentage, 0) / total;
    ...
    const headers = [
        <div className="flex justify-center" key="check-head">
            <input
                ...
                checked={portofolios.data.length > 0 && selectedIds.length === portofolios.data.length}
                className="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 cursor-pointer"
            />
        </div>,
        "JUDUL PORTOFOLIO & KELAS", 
        "JADWAL PENGERJAAN", 
        "PROGRESS", 
        "AKSI"
    ];
    ...
    const rows = portofolios.data.map((portofolio, index) => {
        const isExpired = new Date(portofolio.batas_waktu) < new Date();
        
        // Ambil semester dari kelas pertama
        const semester = portofolio.kelas_harians && portofolio.kelas_harians.length > 0 
            ? portofolio.kelas_harians[0].semester 
            : null;

        return [
            // 1. Checkbox
            <div className="flex justify-center" key={`check-${portofolio.uuid}`}>
                <input
                    type="checkbox"
                    checked={selectedIds.includes(portofolio.uuid)}
                    onChange={() => handleSelectRow(portofolio.uuid)}
                    className="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 cursor-pointer"
                />
            </div>,

            // 2. Judul, Kelas & Semester
            <div className="flex flex-col max-w-sm py-2">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    {/* ORNAMEN SEMESTER */}
                    {semester && (
                        <span className="bg-purple-100 text-purple-600 text-[10px] font-bold px-1.5 py-0.5 rounded border border-purple-200">
                            Sem. {semester}
                        </span>
                    )}
                </div>
                <Link 
                    href={route('my.portofolio.detail', { portofolio: portofolio.uuid })}
                    className="text-base font-bold text-gray-800 hover:text-indigo-600 transition-colors line-clamp-2 leading-tight"
                    title={portofolio.judul}
                >
                    {portofolio.judul}
                </Link>
                <div className="mt-2 flex items-center gap-2">
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800 border border-gray-200">
                        ...
                        {portofolio.kelas_harians_count} Kelas
                    </span>
                </div>
            </div>,
                ...
                <div className="flex items-center gap-2 group">
                    ...
                    <div className="flex flex-col pb-2">
                        ...
                        <span className="text-xs text-gray-600 font-medium font-mono">
                           {portofolio.formatted_waktu_mulai}
                        </span>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                     ...
                     <div className="flex flex-col">
                        ...
                        <span className={`text-sm font-bold font-mono ${isExpired ? 'text-red-600' : 'text-gray-800'}`}>
                           {portofolio.formatted_batas_waktu}
                        </span>
                        {isExpired && (
            ...
            <div className="w-full min-w-[160px] px-2">
                <div className="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <span className="font-bold text-gray-800 bg-gray-100 px-1.5 rounded">
                        {portofolio.progress.total_submissions} / {portofolio.progress.total_students}
                    </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3 shadow-inner">
                    <div
                        className={`h-3 rounded-full transition-all duration-700 ease-out shadow-sm ${
                            portofolio.progress.percentage >= 80 ? 'bg-gradient-to-r from-emerald-400 to-emerald-600' : 
                            portofolio.progress.percentage >= 50 ? 'bg-gradient-to-r from-amber-300 to-amber-500' : 
                ...
                <div className="text-right mt-1">
                     <span className={`text-xs font-bold ${
                         portofolio.progress.percentage >= 80 ? 'text-emerald-700' : 
                         portofolio.progress.percentage >= 50 ? 'text-amber-700' : 'text-rose-700'
                     }`}>
                         {Math.round(portofolio.progress.percentage)}% Selesai
                     </span>
                </div>
            </div>,
            ...
            <div className="flex items-center justify-center gap-2">
                ...
                {canEdit && (
                    <Link 
                        href={route('my.portofolio.edit', { portofolio: portofolio.uuid })} 
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div>
                        <div className="flex items-center text-sm text-gray-500 mb-2 font-medium">
                             <Link href={route('my.portofolio.index')} className="hover:text-indigo-600 transition-colors">
                                <i className="fa fa-folder-open-o mr-1"></i> Portofolio
                             </Link>
                        ...
                        <p className="text-gray-500 mt-2 text-sm max-w-2xl">
                            Kelola daftar portofolio, pantau progress mahasiswa, dan sesuaikan tenggat waktu.
                        </p>
                    </div>
                    ...
                    {stats && (
                        <div className="flex gap-4 min-w-max">
                            <div className="flex flex-col items-center justify-center px-6 py-3 bg-white border border-indigo-100 rounded-xl shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <span className="text-xs font-bold text-gray-500 uppercase tracking-wider mt-1">Total Portofolio</span>
                            </div>
                        ...
                        {selectedIds.length > 0 && canEdit && (
                            <div className="w-full md:w-auto animate-bounce-in">
                                ...
                                >
                                    ...
                                    <span>Update Deadline ({selectedIds.length} Item)</span>
                                </button>
                            </div>
                        )}
                    </div>
