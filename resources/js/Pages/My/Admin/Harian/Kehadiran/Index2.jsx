...
const Index = () => {
    const { mahasiswas, flash, angkatans, selectedAngkatan } = usePage().props; 
            ...
            <tbody>
                {mhs.list_kelas?.map((kelas, idx) => (
                    <tr key={idx}>
                        ...
                        <td className="border text-center px-2 py-1 text-red-600 font-bold">{kelas.alpha}</td>
                        <td className="border text-center px-2 py-1 text-blue-600 font-bold">{kelas.izin}</td>
                        <td className="border text-center px-2 py-1 text-orange-600 font-bold">{kelas.sakit}</td>
                    </tr>
                ))}
            </tbody>
        </table>,
    ...
    const handleAngkatanChange = (e) => {
        ...
        setAngkatan(tahun);
        Inertia.get('/my/harian/status/kehadiran/mahasiswa', { angkatan: tahun }, { preserveState: true });
    };
