...
const Index = () => {
    ...
    const headers = ["No.", "Tahun", "Semester", "Kelas", "Materi", "Actions"];
    ...
    const rows = kelas.data.map((kls, index) => [
        ...
        kls.semester,
        kls.nama_kelas,
        kls.materi_harians_count,
        (
            <div className="flex justify-center">
                {hasAnyPermission(['dsn.dh.mtr.show']) ? (
                    <>
                        {hasAnyPermission(['dsn.dh.mtr.show']) && (
                            <Link
                                // href={`/my/d/materi/${kls.uuid}/show`}
                                href={route('dsn.dh.materi.show', kls.uuid)}
    ...
    return (
        <>
            <Head title="eLearning - Materi Kelas Harian" />
            <MyLayout>
                <div className="flex flex-col mt-5">
                    <div className={`w-full flex flex-col md:flex-row gap-2 items-center ${hasAnyPermission(['dsn.dh.mtr.create']) ? 'justify-between' : 'justify-center'}`}>
                        {hasAnyPermission(['dsn.dh.mtr.create']) && (
                            <Link href={route('dsn.dh.materi.create')}
                        ...
                        <div className="w-full md:w-3/4 lg:w-3/6">
                            <Search URL={'/my/d/harian/materi_harian'} placeholder="Keyword: [Kode Kelas] [Nama Kelas]"/>
                        </div>
                    </div>
                </div>
                <DataTable
