...
const Absen = () => {
    ...
    const absensiHeaders = jadwals.map((jadwal) => {
        const date = new Date(jadwal.tanggal); // Mengubah string tanggal menjadi objek Date
        const options = { weekday: 'long' }; // Mengambil nama hari dalam format panjang (Senin, Selasa, dsb.)
        return date.toLocaleDateString('id-ID', options); // Format tanggal untuk bahasa Indonesia
    });
    // console.log(absensiHeaders);
    ...
    const handleUpdateStatus = (mahasiswaId, jadwalId, absensiId, newStatus) => {
        ...
        // console.log("Kode Kelas:", kelas.id);
