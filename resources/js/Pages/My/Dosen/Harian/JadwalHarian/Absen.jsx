...
const Absen = () => {
    ...
    const handleSetHadirSemua = (jadwalId, tanggal) => {
        Swal.fire({
            title: 'Anda yakin?',
            text: "Aksi ini akan menandai SEMUA mahasiswa yang belum absen sebagai 'Hadir' pada tanggal " + tanggal + ". Status Izin, Sakit, atau Alpha tidak akan berubah.",
            icon: 'question',
                                        ...
                                        {jadwals.map((jadwal, index) => (
                                            <th key={index} className="border px-4 py-2 text-center font-semibold align-top min-w-28">
                                                ...
                                                {hasAnyPermission(['dsn.dh.jadwal.setHadirSemua']) && (
                                                    <button
                                                        onClick={() => handleSetHadirSemua(jadwal.id, jadwal.tanggal)}
                                                ...
                                                {jadwals.map((jadwal, idx) => {
                                                    ...
                                                    return (
                                                        <td key={idx} className="border px-4 py-2 text-center">
                                                            ...
                                                            >
                                                                <option value="-" className={`${hasAbsensi ? 'text-red-600' : 'opacity-50 cursor-not-allowed'}`} disabled={!hasAbsensi}>
                                                                    {hasAbsensi ? 'Hapus' : '-'}                                                                    
                                                                </option>
