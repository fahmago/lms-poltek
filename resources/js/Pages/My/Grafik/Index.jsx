...
// Daftarkan semua plugin Chart.js yang akan digunakan
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Kelas</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {dateList.map((item, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-800 font-mono">{item.date}</td>
                                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{item.className}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg space-y-6">
            <div>
                ...
                <nav className="flex space-x-2 border-b border-gray-200 pb-2 overflow-x-auto">
                    ...
                    {prodis.map((prodi) => (
                        <button key={prodi.id} onClick={() => setActiveProdi(prodi.kode_prodi)} className={`${activeProdi === prodi.kode_prodi ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'} px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap`} title={prodi.nama_prodi}>{abbreviateProdi(prodi.nama_prodi)}</button>
                    ))}
                </nav>
            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-slate-50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">NIM</th>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Nama Mahasiswa</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {recapData.map((mhs, index) => (
                            <tr key={mhs.nim || index} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-sm text-gray-500 font-mono text-left">{mhs.nim || '-'}</td>
...
export default function Index() {
    const { prodis, initialChartData, initialStudentRecap, initialDateRange } = usePage().props;
