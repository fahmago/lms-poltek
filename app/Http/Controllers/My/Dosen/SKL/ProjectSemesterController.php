...
class ProjectSemesterController extends Controller
{
    public function index1()
    {
        ...
        $kelasList = $dosen->kelasHarians()
            ...
            ->withCount('projectSemesters') 
            ->orderBy('tahun', 'desc')
            ->orderBy('semester', 'desc')
            ->paginate(10);

        // GANTI PATH INERTIA
        return inertia('My/Dosen/SKL/Index', [
            'kelasList' => $kelasList,
        ]);
    }
    ...
    public function index(Request $request)
    {
        ...
        // Append query params agar pagination tidak reset
        $kelasList->appends([
            'q' => $request->q,
            'tahun' => $selectedYear,
    ...
    /**
     * Menampilkan daftar project semester untuk satu kelas spesifik.
     */
    public function show($uuid)
    {
        ...
        // Ambil data project dengan pengumpulan
        // GANTI MODEL & RELASI
        $projectSemesters = $kelasHarian->projectSemesters()
            ->with(['pengumpulanProjectSemesters']) // relasi pengumpulan
            ->withCount('kelasHarians')
            ->latest()
            ->paginate(10);

        // GANTI VARIABEL
        $projectSemesters->getCollection()->transform(function ($project) use ($kelasHarian) {
            $totalMahasiswa = $kelasHarian->kelasHarianMahasiswas->count();
            // GANTI RELASI
            $totalPengumpulan = $project->pengumpulanProjectSemesters
        ...
        return inertia('My/Dosen/SKL/Show', [
            'kelasHarian' => $kelasHarian,
            'projectSemesters' => $projectSemesters,
        ]);
    }

    /**
     * Menampilkan detail project dan rekap pengumpulan untuk kelas yang spesifik.
     */
    // GANTI PARAMETER
    public function detail($uuidKelasHarian, $uuidProjectSemester)
    {
        $kelasHarian = KelasHarian::where('uuid', $uuidKelasHarian)->firstOrFail();
        // GANTI MODEL & VARIABEL
        $projectSemester = ProjectSemester::where('uuid', $uuidProjectSemester)->firstOrFail();
        ...
        // GANTI PATH INERTIA & NAMA PROP
        return inertia('My/Dosen/SKL/Detail', [
     ...
     */
    // GANTI PARAMETER
    public function submitGrade(Request $request, $uuidPengumpulanProjectSemester)
    {
        // GANTI MODEL & VARIABEL
        $pengumpulanProjectSemester = PengumpulanProjectSemester::where('uuid', $uuidPengumpulanProjectSemester)->firstOrFail();

        // --- Logika Otorisasi ---
        // GANTI RELASI
        $allowedDosenIds = $pengumpulanProjectSemester->projectSemester
            ->kelasHarians() 
            ->pluck('dosen_id')
            ->unique(); 
