...
class SertifikatController extends Controller
{
    public function index(Request $request)
    {
        ...
        $prodis = Prodi::with([
            // Ganti relasi
            'sertifikats' => function ($query) use ($filterTahun, $filterSemester) {
            ...
            // Ganti relasi
            'sertifikats.kelasHarians.kelasHarianMahasiswas',
            'sertifikats.pengumpulanSertifikats' // Ganti relasi
        ])
        ->withCount(['sertifikats' => function ($query) use ($filterTahun, $filterSemester) { // Ganti relasi
            $query->whereHas('kelasHarians', function ($q) use ($filterTahun, $filterSemester) {
                if ($filterTahun) {
                    $q->where('tahun', $filterTahun);
                }
        ...
        // Kalkulasi progres berdasarkan data sertifikat yang sudah difilter
        $prodis->getCollection()->transform(function ($prodi) use ($filterTahun, $filterSemester) {
            $totalStudents = 0;
            $totalSubmissions = 0;
            
            // Loop melalui sertifikat yang SUDAH DIFILTER
            foreach ($prodi->sertifikats as $tugas) { // Ganti variabel
                ...
                // Ganti relasi
                $submissions = $tugas->pengumpulanSertifikats->unique('mahasiswa_id')->count(); 
        ...
        return inertia('My/Admin/SKL/Sertifikat/Index', [ // Ganti path inertia
            'prodis' => $prodis,
    ...
    public function show(Request $request, $uuid)
    {
        ...
        // Ganti relasi
        $sertifikats = $prodi->sertifikats() 
            ->withCount('kelasHarians')
             // Ganti relasi
            ->with(['kelasHarians.kelasHarianMahasiswas', 'pengumpulanSertifikats'])
        ...
        // Transformasi data sertifikat untuk menghitung progress
        $sertifikats->getCollection()->transform(function ($tugas) { // $tugas adalah sertifikat
            ...
            $totalStudents = $tugas->kelasHarians->flatMap(fn($k) => $k->kelasHarianMahasiswas)->unique('mahasiswa_id')->count();
             // Ganti relasi
            $totalSubmissions = $tugas->pengumpulanSertifikats->unique('mahasiswa_id')->count();
            $completionPercentage = $totalStudents > 0 ? round(($totalSubmissions / $totalStudents) * 100) : 0;
        ...
        return inertia('My/Admin/SKL/Sertifikat/Show', [ // Ganti path inertia
            'prodi' => $prodi,
            'sertifikats' => $sertifikats, // Ganti nama prop
            'angkatans' => $angkatans,
    ...
    public function detail($uuid)
    {
        ...
        $sertifikat->load(['kelasHarians.kelasHarianMahasiswas.mahasiswa.user', 'prodi']);
        ...
        // Ganti variabel
        $rekapPerKelas = $sertifikat->kelasHarians->sortBy('nama_kelas')->map(function ($kelas) use ($submissionsByStudent) {
        ...
        // Ganti variabel
        $totalStudents = $sertifikat->kelasHarians->flatMap(fn($k) => $k->kelasHarianMahasiswas)->unique('mahasiswa_id')->count();
        ...
        return inertia('My/Admin/SKL/Sertifikat/Detail', [ // Ganti path inertia
            'sertifikat' => $sertifikat, // Ganti nama prop
    ...
    public function create()
    {
        ...
        return inertia('My/Admin/SKL/Sertifikat/Create', [ // Ganti path inertia
            'angkatans' => $angkatans,
            'prodis' => $prodis,
        ]);
    }
    ...
    {
        // Validasi (Sama seperti buku, karena migrasi 'sertifikats' identik)
        ...
        try {
            ...
            $sertifikat->kelasHarians()->attach($validatedData['kelas_harian_ids']); // Ganti variabel

            DB::commit();

            return redirect()->route('my.sertifikat.index')->with('success', 'Tugas Sertifikat berhasil dibuat!'); // Ganti route & teks
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Terjadi kesalahan saat menyimpan tugas sertifikat. Silakan coba lagi.'); // Ganti teks
        }
    }
    ...
    public function edit($uuid)
    {
        ...
        $sertifikat->load('kelasHarians', 'prodi');
        ...
        // Ganti variabel
        $firstKelas = $sertifikat->kelasHarians->first();

        return inertia('My/Admin/SKL/Sertifikat/Edit', [ // Ganti path inertia
            'sertifikat' => $sertifikat, // Ganti nama prop
            'angkatans' => $angkatans,
            'prodis' => $prodis,
            'initialFilters' => [
                'tahun_angkatan' => $firstKelas->tahun ?? '',
                'kode_prodi' => $sertifikat->prodi->kode_prodi ?? '', // Ganti variabel
    ...
    public function update(Request $request, $uuid)
    {
        ...
        try {
            ...
            $sertifikat->update([
             ...
             // Ganti variabel
            $sertifikat->kelasHarians()->sync($validatedData['kelas_harian_ids']);

            DB::commit();

            // Ganti route & variabel
            return redirect(session('previous_url'))->with('success', 'Tugas Sertifikat berhasil diperbarui!');
            // return redirect()->route('my.sertifikat.show', $sertifikat->prodi->uuid)->with('success', 'Tugas Sertifikat berhasil diperbarui!');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Terjadi kesalahan saat memperbarui tugas sertifikat.'); // Ganti teks
        }
    }     
    ...
    public function destroy($uuid)
    {
        ...
        try {
            $sertifikat->delete(); // Ganti variabel

            return redirect()->back()->with('success', 'Tugas Sertifikat berhasil dihapus.'); // Ganti teks
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Terjadi kesalahan saat menghapus tugas sertifikat.'); // Ganti teks
        }
    }
    ...
    {
        // Ganti Model dan variabel
        $pengumpulanSertifikat = PengumpulanSertifikat::where('uuid', $uuid)->firstOrFail();
    ...
    public function bulkUpdateDeadline(Request $request)
    {
        $validated = $request->validate([
            ...
            'ids.*' => 'exists:sertifikats,uuid', // Pastikan nama tabel adalah 'sertifikats'
            'batas_waktu' => 'required|date',
        ]);

        try {
            Sertifikat::whereIn('uuid', $validated['ids'])
                ->update(['batas_waktu' => $validated['batas_waktu']]);

            return redirect()->back()->with('success', 'Deadline berhasil diperbarui untuk ' . count($validated['ids']) . ' tugas sertifikat.');
        } catch (\Exception $e) {
